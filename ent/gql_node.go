...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	switch table {
	case launchpad.Table:
		query := c.Launchpad.Query().
			Where(launchpad.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, launchpadImplementors...); err != nil {
				return nil, err
			}
		}
	...
	case mint.Table:
		query := c.Mint.Query().
			Where(mint.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, mintImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case launchpad.Table:
		query := c.Launchpad.Query().
			Where(launchpad.IDIn(ids...))
		query, err := query.CollectFields(ctx, launchpadImplementors...)
		if err != nil {
			return nil, err
		}
	...
	case mint.Table:
		query := c.Mint.Query().
			Where(mint.IDIn(ids...))
		query, err := query.CollectFields(ctx, mintImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
