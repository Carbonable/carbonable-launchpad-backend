...
func (mq *MintQuery) QueryProject() *ProjectQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(mint.Table, mint.FieldID, selector),
...
// First returns the first Mint entity from the query.
// Returns a *NotFoundError when no Mint was found.
func (mq *MintQuery) First(ctx context.Context) (*Mint, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{mint.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first Mint ID from the query.
// Returns a *NotFoundError when no Mint ID was found.
...
// Only returns a single Mint entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one Mint entity is found.
// Returns a *NotFoundError when no Mint entities are found.
func (mq *MintQuery) Only(ctx context.Context) (*Mint, error) {
	...
	case 0:
		return nil, &NotFoundError{mint.Label}
	default:
		return nil, &NotSingularError{mint.Label}
	}
}
...
// OnlyID is like Only, but returns the only Mint ID in the query.
// Returns a *NotSingularError when more than one Mint ID is found.
...
func (mq *MintQuery) OnlyID(ctx context.Context) (id int, err error) {
	...
	case 0:
		err = &NotFoundError{mint.Label}
	default:
		err = &NotSingularError{mint.Label}
	}
	return
}
...
func (mq *MintQuery) All(ctx context.Context) ([]*Mint, error) {
	...
	qr := querierAll[[]*Mint, *MintQuery]()
	return withInterceptors[[]*Mint](ctx, mq, qr, mq.inters)
}
...
func (mq *MintQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if err = mq.Select(mint.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (mq *MintQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, mq, querierCount[*MintQuery](), mq.inters)
}
...
// Clone returns a duplicate of the MintQuery builder, including all associated steps. It can be
...
func (mq *MintQuery) Clone() *MintQuery {
	...
	return &MintQuery{
		config:      mq.config,
		ctx:         mq.ctx.Clone(),
		order:       append([]mint.OrderOption{}, mq.order...),
		inters:      append([]Interceptor{}, mq.inters...),
		predicates:  append([]predicate.Mint{}, mq.predicates...),
...
func (mq *MintQuery) GroupBy(field string, fields ...string) *MintGroupBy {
	...
	grbuild := &MintGroupBy{build: mq}
	grbuild.flds = &mq.ctx.Fields
	grbuild.label = mint.Label
...
func (mq *MintQuery) Select(fields ...string) *MintSelect {
	...
	sbuild := &MintSelect{MintQuery: mq}
	sbuild.label = mint.Label
...
func (mq *MintQuery) prepareQuery(ctx context.Context) error {
...
func (mq *MintQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Mint, error) {
	var (
		nodes       = []*Mint{}
		withFKs     = mq.withFKs
		_spec       = mq.querySpec()
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, mint.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Mint).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &Mint{config: mq.config}
...
func (mq *MintQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Mint, init func(*Mint), assign func(*Mint, *Project)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Mint)
	for i := range nodes {
		if nodes[i].project_mint == nil {
			continue
		}
		fk := *nodes[i].project_mint
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_mint" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MintQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(mint.Table, mint.Columns, sqlgraph.NewFieldSpec(mint.FieldID, field.TypeInt))
	_spec.From = mq.sql
...
func (mq *MintQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(mq.driver.Dialect())
	t1 := builder.Table(mint.Table)
	columns := mq.ctx.Fields
	if len(columns) == 0 {
		columns = mint.Columns
	}
...
func (mgb *MintGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*MintQuery, *MintGroupBy](ctx, mgb.build, mgb, mgb.build.inters, v)
}

func (mgb *MintGroupBy) sqlScan(ctx context.Context, root *MintQuery, v any) error {
...
func (ms *MintSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*MintQuery, *MintSelect](ctx, ms.MintQuery, ms, ms.inters, v)
}

func (ms *MintSelect) sqlScan(ctx context.Context, root *MintQuery, v any) error {
