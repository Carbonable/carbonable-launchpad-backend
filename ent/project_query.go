...
func (pq *ProjectQuery) QueryMint() *MintQuery {
	query := (&MintClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) QueryLaunchpad() *LaunchpadQuery {
	query := (&LaunchpadClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withMint:      pq.withMint.Clone(),
		withLaunchpad: pq.withLaunchpad.Clone(),
...
// WithLaunchpad tells the query-builder to eager-load the nodes that are connected to
// the "launchpad" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithLaunchpad(opts ...func(*LaunchpadQuery)) *ProjectQuery {
	query := (&LaunchpadClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withLaunchpad = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withMint; query != nil {
		if err := pq.loadMint(ctx, query, nodes, nil,
			func(n *Project, e *Mint) { n.Edges.Mint = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProjectQuery) loadMint(ctx context.Context, query *MintQuery, nodes []*Project, init func(*Project), assign func(*Project, *Mint)) error {
	...
	query.withFKs = true
	query.Where(predicate.Mint(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.MintColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_mint
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_mint" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_mint" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProjectQuery) loadLaunchpad(ctx context.Context, query *LaunchpadQuery, nodes []*Project, init func(*Project), assign func(*Project, *Launchpad)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Launchpad(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.LaunchpadColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_launchpad
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_launchpad" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_launchpad" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
